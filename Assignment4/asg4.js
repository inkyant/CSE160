...
function createMap() {
    ...
    let s = new Sphere()
    s.textureBlend = 1.0
    let s_matrix = new Matrix4()
    s_matrix.rotate(90, 1, 0, 0)
    s.matrix = s_matrix
    map.push(s)

    let s2 = new Sphere()
    s.textureBlend = 0.0
    let s2_matrix = new Matrix4()
...
function toggleNormals() {
    var btn = document.getElementById('normals-btn');
    var showing = btn.dataset.showing === 'true';
    showing = !showing;
    btn.dataset.showing = showing;
    btn.textContent = showing ? 'Hide Normals' : 'Show Normals';
    gl.uniform1i(u_ShowNormals, showing ? 1 : 0);
    renderPage();
}
